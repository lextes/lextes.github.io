...
$meta_box = array(
	...
	'fields' => array(
		...
		array(
            ...
            'desc' => 'Please make sure that the image size is 960px x 370px.',
...
add_action('admin_menu', 'boldy_add_box');

// Add meta box
function boldy_add_box() {
	global $meta_box;

	add_meta_box($meta_box['id'], $meta_box['title'], 'boldy_show_box', $meta_box['page'], $meta_box['context'], $meta_box['priority']);
}

// Callback function to show fields in meta box
function boldy_show_box() {
	global $meta_box, $post;

	// Use nonce for verification
	echo '<input type="hidden" name="boldy_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'boldy_save_data');

// Save data from meta box
function boldy_save_data($post_id) {
	global $meta_box;

	// verify nonce
	if (!wp_verify_nonce($_POST['boldy_meta_box_nonce'], basename(__FILE__))) {
		return $post_id;
	}

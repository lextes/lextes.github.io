...
function site5framework_header_init() {
    ...
    wp_deregister_script( 'twittercb' );
	wp_register_script('twittercb', get_template_directory_uri().'/js/twittercb.js');
	wp_enqueue_script( 'twittercb' );
...
function site5framework_custom_shortcut_favicon() {
	if (of_get_option('boldy_custom_shortcut_favicon') != '') {
    echo '<link rel="shortcut icon" href="'. of_get_option('boldy_custom_shortcut_favicon') .'" type="image/ico" />'."\n";
	}
...
function site5framework_custom_icon() {
	if (of_get_option('boldy_custom_shortcut_favicon') != '') {
    echo '<link rel="icon" href="'. of_get_option('boldy_custom_shortcut_favicon') .'" type="image/ico" />'."\n";
	}
...
function site5framework_custom_apple_touch_icon() {
	if (of_get_option('boldy_custom_apple_touch_icon') != '') {
    echo '<link rel="apple-touch-icon" href="'. of_get_option('boldy_custom_apple_touch_icon') .'" type="image/png" />'."\n";
	}
...
function site5framework_custom_apple_touch_icon_precomposed() {
	if (of_get_option('boldy_custom_apple_touch_icon') != '') {
    echo '<link rel="apple-touch-icon-precomposed" href="'. of_get_option('boldy_custom_apple_touch_icon') .'" type="image/png" />'."\n";
	}
...
function site5framework_custom_icon_image() {
	if (of_get_option('boldy_custom_icon_image') != '') {
    echo '<link rel="icon" href="'. of_get_option('boldy_custom_icon_image') .'" type="image/png" />'."\n";
	}
...
function site5framework_portfolio1_init() {
	if (is_page_template('portfolio.php') &&  !is_admin()) {
	wp_enqueue_script('quicksand-js', get_template_directory_uri().'/js/quicksand.js', array('jquery'), '1.0');
    }
}
...
function site5framework_analytics(){
	$output = of_get_option('boldy_stats');
	if ( $output <> "" )
...
function nivo_home_js() {
    if (is_home() && of_get_option('boldy_slidertype') == 'nivo' && !is_admin()) {
